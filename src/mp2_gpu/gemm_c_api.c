   ...
   do {                                                                         \
      ...
      if (status__ != cudaSuccess) {                                            \
         fprintf(stderr, "CUDA_ERROR: %s %s:%d\n",                              \
                 cudaGetErrorString(status__), __FILE__, __LINE__);             \
         abort();                                                               \
      }                                                                         \
   } while (0)
#else
 ...
 ******************************************************************************/
static void blas_dgemm(char transa, char transb,
                       int m, int n, int k,
                       double alpha,
...
static void blas_sgemm(char transa, char transb,
                       int m, int n, int k,
                       float alpha,
...
static void cublas_activate_device(void) {
   offload_activate_chosen_device();
}

static void cublas_dgemm_impl(gemm_ctx_t *ctx,
                              ...
                              double beta,
                              double *C, int ldc) {
   cublasOperation_t opA = (transa == 'N' || transa == 'n') ?
                           CUBLAS_OP_N : CUBLAS_OP_T;
   cublasOperation_t opB = (transb == 'N' || transb == 'n') ?
                           CUBLAS_OP_N : CUBLAS_OP_T;
...
static void cublas_sgemm_impl(gemm_ctx_t *ctx,
                              char transa, char transb,
                              int m, int n, int k,
                              float alpha,
                              const float *A, int lda,
...
static void spla_dgemm_impl(gemm_ctx_t *ctx,
                            char transa, char transb,
                            int m, int n, int k,
                            double alpha,
                            const double *A, int lda,
                            ...
                            double beta,
                            double *C, int ldc) {
   SPLA spLA_op_A = (transa == 'N' || transa == 'n') ?
                    SPLA_OP_NONE : SPLA_OP_TRANSPOSE;
   SPLA spLA_op_B = (transb == 'N' || transb == 'n') ?
                    SPLA_OP_NONE : SPLA_OP_TRANSPOSE;

   SPLA_CHECK(spla_dgemm(spLA_op_A, spLA_op_B,
                         m, n, k, alpha,
                         A, lda,
                         B, ldb,
                         beta, C, ldc,
...
void gemm_ctx_dgemm(gemm_ctx_t *ctx,
...
#if defined(__SPLA) && defined(__OFFLOAD_GEMM)
    case GEMM_LIB_SPLA:
       spla_dgemm_impl(ctx, transa, transb, m, n, k,
                       (double)alpha, (const double *)A, lda,
                       (const double *)B, ldb,
   ...
   case GEMM_LIB_CUBLAS:
      if (ctx->uses_gpu) {
         cublas_dgemm_impl(ctx, transa, transb, m, n, k,
                           alpha, A, lda, B, ldb, beta, C, ldc);
      } else {
...
switch (ctx->lib) {
    ...
    case GEMM_LIB_CUBLAS:
       if (ctx->uses_gpu) {
          cublas_sgemm_impl(ctx, transa, transb, m, n, k,
                           alpha, A, lda, B, ldb, beta, C, ldc);
      } else {
         blas_sgemm(transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
      }
...
const char *gemm_ctx_get_backend_name(gemm_ctx_t *ctx) {
   ...
   switch (ctx->lib) {
#if defined(__CUBLAS)
      case GEMM_LIB_CUBLAS: return ctx->uses_gpu ? "cuBLAS" : "cuBLAS(CPU fallback)";
#endif
#if defined(__SPLA) && defined(__OFFLOAD_GEMM)
      case GEMM_LIB_SPLA:   return ctx->uses_gpu ? "SPLA" : "SPLA(CPU fallback)";
#endif
      case GEMM_LIB_BLAS:   return "BLAS";
      default:              return "unknown";
   }
}

int gemm_ctx_uses_gpu(gemm_ctx_t *ctx) {
   return ctx ? ctx->uses_gpu : 0;
}